...
def improved_model_classification():
    ...
    features = [
        'GF_roll_H', 'GA_roll_H', 'ST_roll_H', 'C_roll_H',
        'GF_roll_A', 'GA_roll_A', 'ST_roll_A', 'C_roll_A',
        'market_diff'
    ]
    ...
    print("--- Improved Model (Logistic Regression Classification) ---")
    print(f"Accuracy: {acc:.4f}\n")
    print(classification_report(y_test, y_pred))

    cm = confusion_matrix(y_test, y_pred, labels=['A', 'D', 'H'])
    plt.figure(figsize=(8, 6))
    sns.heatmap(cm, annot=True, fmt='d', cmap='Greens', xticklabels=['Away', 'Draw', 'Home'], yticklabels=['Away', 'Draw', 'Home'])
    plt.title('Confusion Matrix: Logistic Regression (Classification)')
    plt.savefig(os.path.join(fig_dir, "logit_confusion_matrix.png"))

if __name__ == "__main__":
    improved_model_classification()
